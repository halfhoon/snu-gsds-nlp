...
def reinitialize_layers(model):
    ...
    def init_weights(m):
        ...
        if m.__class__.__name__ == "LSTMCell_assignment":
            m.Wii.weight.data.fill_(0.1)
            m.Wif.weight.data.fill_(0.1)
            m.Wig.weight.data.fill_(0.1)
            m.Wio.weight.data.fill_(0.1)
            m.Whi.weight.data.fill_(0.1)
            m.Whg.weight.data.fill_(0.1)
            m.Whf.weight.data.fill_(0.1)
            m.Who.weight.data.fill_(0.1)

            m.Wii.bias.data.fill_(0.1)
            m.Wif.bias.data.fill_(0.1)
            m.Wig.bias.data.fill_(0.1)
            m.Wio.bias.data.fill_(0.1)
            m.Whi.bias.data.fill_(0.1)
            m.Whg.bias.data.fill_(0.1)
            m.Whf.bias.data.fill_(0.1)
            m.Who.bias.data.fill_(0.1)
...
def generate_outputs(model, source, target, vocab):
    ...
    enc_hiddens_proj = torch.load('./sanity_check_en_es_data/enc_hiddens_proj.pkl', weights_only=False)
    reinitialize_layers(model)
    # Run Tests
    with torch.no_grad():
...
def question_1c_sanity_check(model, src_sents, tgt_sents, vocab):
    """ Sanity check for question 1c. 
        Compares student output to that of model with dummy data.
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1c: Encoder")
    print ("-"*80)


    # Inputs
    Ybar_t = torch.load('./sanity_check_en_es_data/Ybar_t.pkl', weights_only=False)
    dec_init_state = torch.load('./sanity_check_en_es_data/dec_init_state.pkl', weights_only=False)
    enc_hiddens = torch.load('./sanity_check_en_es_data/enc_hiddens.pkl', weights_only=False)
    enc_masks = torch.load('./sanity_check_en_es_data/enc_masks.pkl', weights_only=False)
    enc_hiddens_proj = torch.load('./sanity_check_en_es_data/enc_hiddens_proj.pkl', weights_only=False)

    # Output
    dec_state_target = torch.load('./sanity_check_en_es_data/dec_state.pkl', weights_only=False)
    ...
    assert(np.allclose(model_results[0].detach().numpy(),torch_results[0].detach().numpy())), "decoder_state[0] is incorrect: it should be:\n {} but is:\n{}".format(model_results[0], torch_results[0])
    print("dec_state[0] Sanity Checks Passed!")
    assert(model_results[1].shape == torch_results[1].shape), "decoder_state[1] shape is incorrect: it should be:\n {} but is:\n{}".format(model_results[1].shape, torch_results[1].shape)
    assert(np.allclose(model_results[1].detach().numpy(), torch_results[1].detach().numpy())), "decoder_state[1] is incorrect: it should be:\n {} but is:\n{}".format(model_results[1], torch_results[1])
    ...
    print ("-"*80)    
    print("All Sanity Checks Passed for Question 1c: Encoder!")
    print ("-"*80)
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Load Outputs
    enc_hiddens_target = torch.load('./sanity_check_en_es_data/enc_hiddens.pkl', weights_only=False)
    dec_init_state_target = torch.load('./sanity_check_en_es_data/dec_init_state.pkl', weights_only=False)
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    enc_hiddens = torch.load('./sanity_check_en_es_data/enc_hiddens.pkl', weights_only=False)
    enc_masks = torch.load('./sanity_check_en_es_data/enc_masks.pkl', weights_only=False)
    target_padded = torch.load('./sanity_check_en_es_data/target_padded.pkl', weights_only=False)

    # Load Outputs
    combined_outputs_target = torch.load('./sanity_check_en_es_data/combined_outputs.pkl', weights_only=False)

    # Configure for Testing
    reinitialize_layers(model)
    COUNTER = [0]
    def stepFunction(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks):
        dec_state = torch.load('./sanity_check_en_es_data/step_dec_state_{}.pkl'.format(COUNTER[0]), weights_only=False)
        o_t = torch.load('./sanity_check_en_es_data/step_o_t_{}.pkl'.format(COUNTER[0]), weights_only=False)
        COUNTER[0]+=1
...
def main():
    ...
    if args['1d']:
        ...
        question_1c_sanity_check(model, src_sents, tgt_sents, vocab)
    elif args['1e']:
